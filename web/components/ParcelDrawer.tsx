...
// In-engine "why this roof" summary: generated by the Bedrock LLM registered in
// Lucenia ml-commons (creds stay in-cluster), grounded in the parcel's own fields.
// Fetched on demand when the drawer opens for a parcel.
// Cache summaries per parcel for the page session so reopening a drawer doesn't
// re-generate (each generation is a billable LLM call).
const whyCache = new Map<string, { text: string; source?: string }>();

function WhyThisRoof({ id }: { id?: string }) {
  const [state, setState] = useState<{ loading: boolean; text?: string; source?: string; hide?: boolean }>({ loading: true });
  useEffect(() => {
    if (!id) return;
    const cached = whyCache.get(id);
    if (cached) { setState({ loading: false, text: cached.text, source: cached.source }); return; }
    let alive = true;
    setState({ loading: true });
    fetch(`/api/why/${encodeURIComponent(id)}`)
      .then(async (r) => {
        const data = await r.json().catch(() => ({}));
        if (!alive) return;
        if (r.ok && data.summary) {
          whyCache.set(id, { text: data.summary, source: data.source });
          setState({ loading: false, text: data.summary, source: data.source });
        } else setState({ loading: false, hide: true }); // no LLM available: show nothing
      })
      .catch(() => alive && setState({ loading: false, hide: true }));
    return () => { alive = false; };
  }, [id]);

  if (state.hide) return null;
  const label = state.source === "openai-fallback" ? "OpenAI (fallback)" : "Lucenia + Bedrock";
  return (
    <div data-testid="why-this-roof" className="rounded-[var(--radius)] border border-accent/40 bg-accent/5 p-3">
      <h4 className="flex items-center gap-1.5 text-sm font-semibold">
        <span aria-hidden>✨</span> Why this roof
        <span className="ml-auto text-[10px] font-normal text-muted">{state.loading ? "generating…" : label}</span>
      </h4>
      {state.loading ? (
        <div className="mt-2 space-y-1.5" aria-label="Generating summary">
          <div className="h-3 w-full animate-pulse rounded bg-border" />
          <div className="h-3 w-4/5 animate-pulse rounded bg-border" />
        </div>
      ) : (
        <p className="mt-1.5 text-sm leading-snug">{state.text}</p>
      )}
    </div>
  );
}
